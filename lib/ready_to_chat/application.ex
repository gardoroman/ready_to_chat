defmodule ReadyToChat.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      ReadyToChat.Repo,
      # Start the Telemetry supervisor
      ReadyToChatWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: ReadyToChat.PubSub},
      #Start the Presence module
      #Note: must be setup before Endpoint
      ReadyToChatWeb.Presence,
      # Start the Endpoint (http/https)
      ReadyToChatWeb.Endpoint
      # Start a worker by calling: ReadyToChat.Worker.start_link(arg)
      # {ReadyToChat.Worker, arg}
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: ReadyToChat.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  def config_change(changed, _new, removed) do
    ReadyToChatWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
